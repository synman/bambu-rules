...
2. **Run the bpm → bambu-mcp traceability & coverage audit.** Every item in bpm's public API must be reachable through at least one access path in the mcp: an MCP tool, the mcp's HTTP REST API, or both. Knowledge modules must accurately document all covered items. All High-severity gaps (Types A, C, D, F — unreachable fields, wrong documentation, or broken coverage) must be resolved before baseline. Medium/Low gaps (Types B, E, G — undocumented but accessible, intentionally unwrapped, or asymmetric coverage) must be documented as known issues and may not be silently carried forward. **Deprecated items with no defined replacement are automatically excluded from all coverage gap assessments** — do not flag them as gaps and do not require them to appear in any intentional exclusions table. If a replacement is defined, the replacement must be covered instead. See the gap audit plan for the full taxonomy and procedure.
