...
| `url-factory-gzip-threshold` | 2026-03-11 | **v1.1.0** | `4c3ac821` | URL factory pattern (4 tools → HTTP URL); gzip threshold 100K→300 chars; 3 new monitoring HTTP routes; Knowledge Completeness Obligation rule; Plan Review and Share rule |
...
**`url-factory-gzip-threshold` — repo SHAs:**
| Repo | Branch | SHA |
|------|--------|-----|
| `~/bambu-printer-manager` | `devel` | `4c3ac821c6498ba1c8c11020acdf9ec6c16329b6` |
| `~/bambu-printer-app` | `main` | `7d8c60fecb41d2aa7f80e755033e1317e080c25c` |
...
- **Lateral impact assessment (mandatory for every rules change — addition or modification):** Scan all other rules sections for rules whose scope, authority, or effective meaning would be inadvertently narrowed, widened, superseded, or contradicted by the proposed change. A change that is correct in isolation but silently alters the meaning of a rule elsewhere is not a safe change. If lateral impact is found, address it explicitly before writing anything. This is the butterfly effect check — small, targeted changes can have unintended cascading consequences across the rules corpus that no individual section review would catch.
...
**Identity resolution (MCP-First + Vault-First — mandatory, no hardcoding):**
- **Hostname**: `federation_status` → top-level `instance_id` field. Vault-authoritative (`pref-instance-id`); `federation_status` reads it fork-free via MCP.
