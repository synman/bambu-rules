...
└── baselines/
    └── <name>.<repo>.copilot-instructions.md   # Point-in-time per-repo rules snapshots
    └── copilot-instructions.<name>.md          # Global rules snapshot for each baseline
```

**Each `projects/<repo>/` directory contains only `copilot-instructions.md`.** Knowledge module source files (Python, JSON, etc.) live in the repo they belong to — never here.
...
**Naming convention:** Use descriptive names reflecting what changed, not temporal state. Examples: `post-audit-clean`, `immutable-guard`, `change-lifecycle-standard`. Never use `current-state` or `latest`.

**SemVer versioning:** Every baseline carries a `vMAJOR.MINOR.PATCH` git tag applied to all in-scope repos. `bambu-printer-manager` is **excluded from tagging** (BPM Write Scope Lock) — its SHA is recorded in the Known Baselines table in the global rules file instead.

| Increment | When |
|-----------|------|
| **MAJOR** | Breaking change to a core API, protocol, or rules structure |
| **MINOR** | New feature, new coverage additions, or meaningful behavioral rule change (most baselines) |
| **PATCH** | Rules-only fix, typo, or doc correction with no functional change |

**Pre-flight requirements** (all mandatory — see global rules for full detail):
1. Post-audit complete — all behavioral gaps written into rules files before capture
2. bpm → mcp traceability & coverage audit — all High-severity gaps resolved
3. All workspace repos clean (`git status --short` shows nothing)
4. All workspace repos pushed (no `ahead`)
...
- **Change Lifecycle Standard** — Stages 1–7 apply to **every** behavior-affecting change (bugs, enhancements, todos, knowledge updates, rules changes). Not scoped to bugs only. Stage 7 commit gate: `py_compile` + `mcp-reload` + Stage 6 verification must all pass in the current turn before `git commit` is allowed.
  - Stage 1: Record — GitHub issue filed immediately, no deferral
  - Stage 2: Triage — severity classification (Blocking / Degraded / Cosmetic)
  - Stage 3: Research Gate — root cause in source, sibling pattern check, scope boundary
  - Stage 4: Plan — mandatory test protocol (pre-fix, post-fix, Tier 1 static); write-block test split: blocked path = agent-autonomous, permitted path = `ask_user` gate required
  - Stage 5: Implementation — `py_compile` first, then deploy (`mcp-reload`) before any test phases
  - Stage 6: Verification — pre-fix baseline (mandatory, non-recoverable once deployed), post-fix delta, fallback path; completeness trace required for knowledge/rules changes
  - Stage 7: Delivery — commit gate (see above), push, rules update if gap found, GitHub close with before/after evidence, bambu-rules sync
...
- Knowledge Module Maintenance Standard — when and how to update baked-in knowledge modules; behavioral completeness trace required before marking any knowledge change done
- Chamber Light Protocol — auto-manage pattern for camera ops: check → set ON if needed → run → restore state; `set_chamber_light` requires `user_permission=True`
